    ...
    function makeElementList($table, $conf, $tags, $tagUid)
    {
        ...
        if ($this->pi1Vars['searchWord'] && $conf['taggedElements.']['searchFields.'][$table]) {
            ...
            $searchTags = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                    'uid', 'tx_tagpack_tags', 'name LIKE ' . $GLOBALS['TYPO3_DB']->fullQuoteStr('%' . $this->pi1Vars['searchWord'] . '%', 'tx_tagpack_tags')
            );
        ...
        $taggedElements = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                $table . '.*,COUNT(tx_tagpack_tags_relations_mm.uid_foreign) AS counter', $table . ' JOIN tx_tagpack_tags_relations_mm ON (tx_tagpack_tags_relations_mm.uid_foreign=' . $table . '.uid AND tx_tagpack_tags_relations_mm.tablenames=\'' . $table . '\')', $table . '.pid>0
				' . $this->cObj->enableFields($table) . '
				' . $calendarSettings . $searchSettings . $tagsSelected, $table . '.uid', (($conf['taggedElements.']['sortFields.'][$table]) ? $table . '.' . $conf['taggedElements.']['sortFields.'][$table] : 'counter DESC,' . $table . '.uid,' . $table . '.' . $sortingTime . ' DESC'), $limit);
        if (!$GLOBALS['TYPO3_DB']->sql_error()) {
            if (count($tags)) {
                foreach ($tags as $key => $tag) {
                    if ($tag['uid'] == t3lib_div::_GET('tx_tagpack_pi3_removeItems')) {
                        ...
                        $newUidList .= $newUidList ? ',' . $tag['uid'] : $tag['uid'];
                    }
                }
            }
            $filters = $conf['taggedElements.']['additionalFilters.'][$table . '.'];
            if (count($tags)) {
                foreach ($tags as $key => $tag) {
                    ...
                    $tagname .= $tagname ? (($key + 1) < count($tags) ? ', ' . $tagLinka : ' ' . $this->pi_getLL('and') . ' ' . $tagLink) : $tagLink;
                }
            }
            if (count($filters)) {
                foreach ($filters as $fieldName => $filterSettings) {
                ...
                foreach ($filters as $fieldName => $filterSettings) {
                    ...
                    if ((!$taggedElement[$fieldName] && $getVar) || ($taggedElement[$fieldName] != $getVar) && $getVar) {
                        ...
                        $availableElementsSelect = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query(
                                $table . '.uid', $table, $filterSettings['mm_table'], $filterSettings['foreign_table'], ' AND ' . $table . '.uid=' . intval($taggedElement['uid']) . ' AND ' . $filterSettings['foreign_table'] . '.uid=' . intval($getVar));
