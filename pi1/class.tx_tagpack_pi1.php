    ...
    function makeTagCloud($content, $conf)
    {
        ...
        if (count($conf)) {
            if ($this->piVars['filtermode'] === 'on' && !$conf['singleItemCloud'] && ($this->piVars['uid'] || $this->piVars['searchWord'])) {
                ...
                if ($this->piVars['searchWord']) {
                    $searchWordMatchTags = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                            'uid', 'tx_tagpack_tags', 'name LIKE ' . $GLOBALS['TYPO3_DB']->fullQuoteStr('%' . $this->piVars['searchWord'] . '%', 'tx_tagpack_tags') . ' AND NOT deleted AND NOT hidden'
                    );
                    ...
                    foreach ($selectedTags as $key => $selectedUid) {
                        if ($selectedUid != t3lib_div::_GET('tx_tagpack_pi3_removeItems')) {
                            $taggedItems = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                                    'tx_tagpack_tags_relations_mm.uid_foreign,tx_tagpack_tags_relations_mm.tablenames', 'tx_tagpack_tags_relations_mm JOIN tx_tagpack_tags ON (' . $table . $uid . 'tx_tagpack_tags.uid=tx_tagpack_tags_relations_mm.uid_local AND tx_tagpack_tags_relations_mm.uid_local=' . intval($selectedUid) . ')', $pid . 'NOT tx_tagpack_tags.deleted AND NOT tx_tagpack_tags.hidden AND NOT tx_tagpack_tags_relations_mm.deleted AND NOT tx_tagpack_tags_relations_mm.hidden', '');
                            if (!$GLOBALS['TYPO3_DB']->sql_error()) {
            ...
            $tagRelations = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                    'DISTINCT tx_tagpack_tags.relations', 'tx_tagpack_tags_relations_mm JOIN tx_tagpack_tags ON (' . $table . $uid . 'tx_tagpack_tags.uid=tx_tagpack_tags_relations_mm.uid_local)', $pid . $filteritems . 'NOT tx_tagpack_tags.deleted AND NOT tx_tagpack_tags.hidden AND NOT tx_tagpack_tags_relations_mm.deleted AND NOT tx_tagpack_tags_relations_mm.hidden', '', 'tx_tagpack_tags.relations DESC', intval($conf['maxNumberOfSizes']));
            ...
            $tagArray = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                    'tx_tagpack_tags.uid,tx_tagpack_tags.name,tx_tagpack_tags.relations', 'tx_tagpack_tags_relations_mm JOIN tx_tagpack_tags ON (' . $table . $uid . 'tx_tagpack_tags.uid=tx_tagpack_tags_relations_mm.uid_local)', $pid . $filteritems . 'NOT tx_tagpack_tags.deleted AND NOT tx_tagpack_tags.hidden AND tx_tagpack_tags.relations>=' . $min . ' AND NOT tx_tagpack_tags_relations_mm.deleted AND NOT tx_tagpack_tags_relations_mm.hidden', 'tx_tagpack_tags.uid', 'tx_tagpack_tags.name ASC', '');
